...
import com.wapo.flagship.features.grid.model.CarouselImmersionItem
...
import com.washingtonpost.android.recirculation.databinding.CarouselImmersionItemBinding
import com.washingtonpost.android.sections.R
import java.util.concurrent.TimeUnit

class CarouselImmersionHolder(
    itemView: View,
    val requestListener: CarouselProvider,
    val nightModeEnabled: Boolean,
    val parent: ViewGroup
) : GridViewHolder(itemView) {
    private lateinit var carouselImmersion: CarouselImmersion
    ...
    override fun bind(position: Int, gridAdapter: GridAdapter) {
        carouselImmersion = gridAdapter.items[position] as CarouselImmersion
        compoundLabel = carouselImmersion.label
        ...
        val isFullWidth = carouselImmersion?.resolvedColumnSpan == (parent as? WPGridView)?.getColumnCount()
        ...
        if (carouselImmersion.label == null) {
            compoundLabelView.setVisible(false)
        } else {
            if (isSmallerBreakPoint) {
                carouselImmersion.label?.secondaryText = "1 of ${carouselImmersion.items.size}"
            } else {
                carouselImmersion.label?.secondaryText = null
            }
            compoundLabelView.setLabel(carouselImmersion.label)
            compoundLabelView.setVisible(true)
        }

        if (carouselImmersion.cta == null) {
            ctaView.setVisible(false)
        } else {
            ctaView.setLabel(carouselImmersion.cta)
            ctaView.setVisible(true)
            ctaView.setOnClickListener {
                carouselImmersion.cta?.let {
        ...
        val clickListener: OnCarouselClickedListener = object : OnCarouselClickedListener {
            override fun onCardClicked(url: String?, positionInCarousel: Int) {
                ...
                if (isSmallerBreakPoint && positionInCarousel == carouselImmersion.items.size) {
                    // end card
                    url.let {
                        carouselImmersion.cta?.let {
                    ...
                    gridAdapter.onCarouselImmersionCardClicked
                        ?.invoke(carouselImmersion.items.map { getLink(it) ?: it.link }, positionInCarousel)
        ...
        val mediaAspectRatio = (carouselImmersion.items.minOfOrNull { it.media?.aspectRatio ?: 1f }) ?: 1f
        carouselView.initLayout(
            ...
            isNewsprint = carouselImmersion.isNewsprint
        )

        // Set Data to CarouselView
        val immersionCarouselItems: MutableList<CarouselViewItem>? =
            MutableList(carouselImmersion.items.size) { index ->
                val item = carouselImmersion.items[index]
                val art = carouselImmersion.items[index].media
                val link = carouselImmersion.items[index].link
                val headline = carouselImmersion.items[index].headline
                val byLine = carouselImmersion.items[index].signature ?: ""
                val secondaryLabel = carouselImmersion.items[index].secondaryText
                val kicker = carouselImmersion.items[index].kicker
                val liveImage = carouselImmersion.items[index].liveImage ?: false
                val style = carouselImmersion.items[index].style
                ...
                CarouselImmersionViewItem(
                    ...
                    linkUrl,
                    null,
                    carouselImmersion.label?.text,
                    kicker,
                    byLine,
                    link.type == LinkType.WEB,
                    headline.prefix,
                    liveImage,
                    carouselImmersion.cardify,
                    refreshInterval,
                    style,
                    secondaryLabel
                )
        ...
        calculateMinCardHeight(immersionCarouselItems)
        ...
        if (isSmallerBreakPoint) {
            carouselImmersion.cta?.let {
                if (it.link?.url != null && it.text != null) {
                    ...
                    it.link?.let { link ->
                        val endCard = CarouselEndCardViewItem(link.url, it.text, carouselImmersion.cardify)
                        immersionCarouselItems?.add(endCard)
                    }
                }
            }
                ...
                override fun onScrollStateChanged(recyclerView: RecyclerView, newState: Int) {
                    ...
                    if (newState == RecyclerView.SCROLL_STATE_IDLE) {
                        ...
                        if (compoundLabel != null && isSmallerBreakPoint) {
                            val currentPosition = if (newScrollPosition > carouselImmersion.items.size) carouselImmersion.items.size else newScrollPosition
                            if (currentPosition > 0) {
                                compoundLabel?.secondaryText = "$currentPosition of ${carouselImmersion.items.size}"
        ...
        if (carouselImmersion.isNewsprint) {
            NewsprintHelper.applyNewsprintGradient(itemView, carouselImmersion)
            compoundLabelView.setTextColor(Color.WHITE)
            compoundLabelView.updateKickerLabelColors(Color.WHITE)
        }
    }

    private fun getLink(item: CarouselImmersionItem?): Link? {
        item ?: return null
    ...
    private fun calculateMinCardHeight(carouselViewItems: List<CarouselViewItem>?) {
        ...
        val cardWidth = itemView.resources.getDimensionPixelSize(com.washingtonpost.android.recirculation.R.dimen.carousel_immersion_card_width) - cardVerticalMargins
        ...
        carouselViewItems?.forEach { item ->
            val kickerTextHeight = if (!item.kicker.isNullOrEmpty())
                TextMeasure().measureTextHeight(
                    ...
                    cardWidth,
                    itemView.context,
                    com.washingtonpost.android.recirculation.R.style.immersion_carousel_item_kicker
                ) else 0
            val secondaryTextHeight = if (!item.secondaryLabel.isNullOrEmpty())
                TextMeasure().measureTextHeight(
                    ...
                    cardWidth,
                    itemView.context,
                    com.washingtonpost.android.recirculation.R.style.immersion_carousel_item_byline
                ) else 0
            ...
            val imageReqH = if (item is CarouselImmersionViewItem && item.liveImage == true && item.artAspectRatio != null)
        ...
        tallestItem?.let {
            val dummyView: CarouselImmersionItemBinding =
                CarouselImmersionItemBinding.inflate(LayoutInflater.from(itemView.context))
            val imageReqH = if (it is CarouselImmersionViewItem && it.liveImage == true && it.artAspectRatio != null) {
                dummyView.itemImmersionArt.visibility = View.GONE
                (cardWidth / (it.artAspectRatio ?: 1f)).toInt()
            } else {
                dummyView.itemImmersionArt.visibility = View.VISIBLE
                0
            }
            if (!it.kicker.isNullOrEmpty()) {
                dummyView.immersionKicker.visibility = View.VISIBLE
                dummyView.immersionKicker.text = it.kicker
            } else dummyView.immersionKicker.visibility = View.GONE
            if (!it.secondaryLabel.isNullOrEmpty()) {
                dummyView.secondaryLabel.visibility = View.VISIBLE
                dummyView.secondaryLabel.text = it.kicker
            } else dummyView.secondaryLabel.visibility = View.GONE
            if (it.title.isNotEmpty()) {
                dummyView.immersionHeadlineTitle.visibility = View.VISIBLE
                dummyView.immersionHeadlineTitle.text = it.title
            } else dummyView.immersionHeadlineTitle.visibility = View.GONE
            if (it.byline.isNotEmpty()) {
                dummyView.byline.visibility = View.VISIBLE
                dummyView.byline.text = it.byline
            } else dummyView.byline.visibility = View.GONE
            ...
            val measuredHeight = if (dummyView.itemImmersionArt.visibility == View.GONE
                && dummyView.immersionKicker.visibility == View.GONE
                && dummyView.secondaryLabel.visibility == View.GONE
                && dummyView.immersionHeadlineTitle.visibility == View.GONE
                && dummyView.byline.visibility == View.GONE
    ...
    companion object {
        private val TAG = CarouselImmersionHolder::class.java.toString()
        ...
        const val IMMERSION_CAROUSEL_RECIRC = "immersion_carousel_recirc"
        const val IMMERSION_CAROUSEL_END_CARD = "immersion_carousel_end_card"
    }
}