...
public class SelectableTextView extends TextViewSpanFix implements Selectable, ITextView {
    ...
    @Override
    public float[] getScreenPositionForOffset(int offset, final float[] position){
        ...
        Rect rect = getScreenRect(null);
        float scaleX = getWidth() == 0 ? 0 : (float) rect.width() / getWidth();
        float scaleY = getHeight() == 0 ? 0 : (float) rect.height() / getHeight();
        position[0] = rect.left + (getLayout().getPrimaryHorizontal(offset) + getCompoundPaddingRight()) * scaleX;
        position[1] = rect.top + getLayout().getLineBottom(line) * scaleY;
        return position;
    }
    ...
    public void selectText(int start, int end){
        if (startSelection == start && endSelection == end || getLayout() == null) {
            return;
        }
