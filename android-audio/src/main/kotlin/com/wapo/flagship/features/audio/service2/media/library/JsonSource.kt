...
internal class JsonSource(private val source: Uri) : AbstractMusicSource() {
    ...
    companion object {
        const val ORIGINAL_ARTWORK_URI_KEY = "com.example.android.JSON_ARTWORK_URI"
    }
...
/**
 * Extension method for [MediaMetadata.Builder] to set the fields from
...
fun MediaMetadata.Builder.from(jsonMusic: JsonMusic): MediaMetadata.Builder {
    ...
    extras.putString(METADATA_KEY_MEDIA_ID, jsonMusic.id)
    this.setTitle(jsonMusic.title)
    this.setDisplayTitle(jsonMusic.title)
    extras.putString(METADATA_KEY_ARTIST, jsonMusic.artist)
    extras.putString(METADATA_KEY_ALBUM, jsonMusic.album)
    extras.putLong(METADATA_KEY_DURATION, durationMs)
    this.setGenre(jsonMusic.genre)
    extras.putString(METADATA_KEY_ALBUM_ART_URI, jsonMusic.image)
    extras.putLong(METADATA_KEY_TRACK_NUMBER, jsonMusic.trackNumber)
    extras.putString(METADATA_KEY_DISPLAY_SUBTITLE, jsonMusic.artist)
    extras.putString(METADATA_KEY_DISPLAY_DESCRIPTION, jsonMusic.album)
    extras.putString(METADATA_KEY_DISPLAY_ICON_URI, jsonMusic.image)
    this.setIsPlayable(true)
    this.setIsBrowsable(true)
    return this
}
