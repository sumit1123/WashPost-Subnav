...
internal class PackageValidator(context: Context, @XmlRes xmlResId: Int) {
    ...
    companion object {
        private const val DEFAULT_PACKAGE_ID = -1
    }
    ...
    private fun buildCallerInfo(callingPackage: String): CallerPackageInfo? {
        ...
        val uid = packageInfo.applicationInfo?.uid ?: DEFAULT_PACKAGE_ID
        ...
        requestedPermissions?.forEachIndexed { index, permission ->
            ...
            if (permissionFlags?.isNotEmpty() == true && permissionFlags[index] and REQUESTED_PERMISSION_GRANTED != 0) {
    ...
    private fun getSignature(packageInfo: PackageInfo): String? =
        if (packageInfo.signatures == null || packageInfo.signatures?.size != 1) {
            ...
            val certificate = packageInfo.signatures?.get(0)?.toByteArray()
            if (certificate != null) {
    ...
    private fun getSignatureSha256(certificate: ByteArray): String {
        ...
        try {
            ...
            Logger.e(TAG, "No such algorithm: $noSuchAlgorithmException")
