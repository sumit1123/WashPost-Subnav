...
public class Global
{
	public static String mCompany = Utils.getCompanyNameForPDFLibrary();

	/**
	 * Key and email for the library
	 */
	public static String mEmail = Utils.getAdminEmailForPDFLibrary();
	public static String mKey = Utils.getKeyForPDFLibrary();
	 ...
	 */
	public static int readerViewBgColor = 0x00000000;
	/**
...
//		load_cmaps( res, R.raw.cmaps, new File(files, "cmaps"), R.raw.umaps, new File(files, "umaps") );
		...
		//int licenseType = active(act, mKey);
		int licenseType = 0;
...
//		load_truetype_font( res, R.raw.amiri_regular, new File(files, "amiriRegular.ttf") );//arabic
		...
		while (face_first < face_count)
		{
			...
			//LogUtil.d("------Fonts------", "----face name = " + face_name);
