...
/**
 * Created by elamgodilj on 8/3/17.
 */
...
public class WapoPDFPageView extends View
{
    ...
    static private Paint m_info_paint;
    ...
    public WapoPDFPageView(Context context)
    {
        super(context);
        init();
    }
    public WapoPDFPageView(Context context, AttributeSet attrs)
    {
        super(context, attrs);
        init();
    }
    public WapoPDFPageView(Context context, String defaultPDFPath) {
        super(context);
     ...
     */
    public void vOpen(VThread thread, VThread thread_cache, Document doc, int pageno, int fit_type, WapoPDFViewPager.WapoReaderListener listener)
    {
        vClose();
        m_scale = 0;
        m_x = 0;
    ...
    protected void onDraw(Canvas canvas)
    {
        ...
        if(m_bmp == null)
        {
            try {
                ...
                Logger.e("WapoPDFPageView", "OOM ", e);
                return;
            }
        }
        m_bmp.eraseColor(Global.readerViewBgColor);
        ...
        if(m_zooming)
        {
           ...
           /* Canvas bcan = new Canvas(m_bmp);
            m_vpage.vDraw(m_thread, bcan, m_x, m_y);
            if(Global.dark_mode)
            {
                BMP bmp = new BMP();
                bmp.Create(m_bmp);
...
//            canvas.drawBitmap(m_bmp, 0, 0, null);
    ...
    public boolean onTouchEvent(MotionEvent event)
    {
        ...
        try {
            if (!defaultPDFPath.equals(getTag(R.id.print_page_tag)) && onTouchZoom(event)) {
        ...
        {
            Logger.e("WapoPDFPageView", "exception ", e);
            return false;
        }
    }
    ...
    class PDFGestureListener extends GestureDetector.SimpleOnGestureListener
    {
        ...
        public boolean onSingleTapConfirmed(MotionEvent e)
        {
            ...
            if (page != null){
                Page.Annotation m_annot = page.GetAnnotFromPoint(GetPDFX(e.getX()), GetPDFY(e.getY()));
                if (m_annot != null){
                    String uri = m_annot.GetURI();
                    if (uri != null){
                        if (listener != null){
                            listener.onOpenURI(uri, true);
                            return true;
                        }
                    }
                }
