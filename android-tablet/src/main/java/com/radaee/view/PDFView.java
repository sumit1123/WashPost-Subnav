	...
	public void vDraw( Canvas canvas )
	{
		...
		if( m_drawbmp )
		{
			if( Global.dark_mode )
			{
				...
				//LogUtil.i"time_d1", String.valueOf(System.currentTimeMillis() - rec_time));
				canvas.drawBitmap(m_bmp, 0, 0, null);
				//LogUtil.i"time_d2", String.valueOf(System.currentTimeMillis() - rec_time));
			}
			else
			{
		...
		{
			m_bmp.eraseColor(m_back);
			//LogUtil.i"time_d0", String.valueOf(System.currentTimeMillis() - rec_time));
			m_draw_bmp.Create(m_bmp);
			while( cur < end )
			{
				PDFVPage vpage = m_pages[cur];
		...
		//LogUtil.i"time_draw", String.valueOf(System.currentTimeMillis() - rec_time));
	}
