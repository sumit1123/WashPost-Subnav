 ...
 */
public class SelectableTextViewOld extends TextView implements SelectableOld {
    ...
    public SelectableTextViewOld(Context context) {
        super(context);
    }

    public SelectableTextViewOld(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public SelectableTextViewOld(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public float[] getPositionForOffset(int offset, final float[] position){
        ...
        position[0] = getLayout().getPrimaryHorizontal(offset) + location[0];
    ...
    public void selectText(int start, int end){
        ...
        if (start == 0 && end == getText().length()) {
            ...
            rectMiddle.left = 0;
            rectMiddle.top = 0;
            rectMiddle.right = getWidth();
            rectMiddle.bottom = getHeight();
    ...
    public CharSequence getSelectedText() {
        if (startSelection >= 0 && endSelection >=0 && startSelection <= endSelection)
            return getText().subSequence(startSelection, endSelection);
        else
            return "";
    }
