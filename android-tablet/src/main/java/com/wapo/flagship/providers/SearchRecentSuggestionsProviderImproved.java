...
/**
 * Fork of {@link android.content.SearchRecentSuggestionsProvider} from v17 which allows custom icons for suggestions.
 *
 * if merging up to newer version note that changes from the base version have been made to add #mIcon and it's setter,
 * as well as it's usage in #setupSuggestions
 *
...
public class SearchRecentSuggestionsProviderImproved extends ContentProvider {
    ...
    private int mIcon = R.drawable.ic_search_history;
    ...
    private static class DatabaseHelper extends SQLiteOpenHelper {
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
            Logger.w(TAG, "Upgrading database from version " + oldVersion + " to "
    ...
    /**
     * Change the icon used for suggestions.  To use this you must call it before setupSuggestions
     * (or add it as a parameter to setup suggestions)
     * @param icon Drawable Resource ID to use
     */
    ...
    /**
     * Convenience method for {@link #setupSuggestions(String, int)}
     * @param authority See {@link #setupSuggestions(String, int)}
     * @param mode See {@link #setupSuggestions(String, int)}
     * @param icon Drawable Resource Identifier for the icon to show on each suggestion
     */
    protected void setupSuggestions(String authority, int mode, int icon){
        setIcon(icon);
