...
import paywall.test.utils.CustomAuthTestApplication;

@RunWith(AndroidJUnit4.class)
public class AuthStateManagerTest {

    private AuthStateManager authStateManager;
    private final static String AUTH_URL = "https://www.washingtonpost.com/subscribe/stage/signin/?case=noa";
    private final static String TOKEN_URL = "https://idstg.washingtonpost.com/identity/oauth/v1/token";
    private final static String CLIENT_ID = "SMACDFC25F524844E0530100007FADB4";
    private final static String APP_REDIRECT_SCHEME = CustomAuthTestApplication.APP_REDIRECT_SCREEN;
    private final static String SCOPE = "profile_access_scope";
    private final static String STATE = UUID.randomUUID().toString();
    private final static String TOKEN_TYPE_BEARER = "Bearer";
    private final static String TOKEN_TYPE_REFRESH = "refresh_token";
    ...
    public void setUp() throws IOException {
        ...
        androidx.test.platform.app.InstrumentationRegistry.getInstrumentation().runOnMainSync(() ->
                PaywallService.initialize(
                        ...
                        new AbstractStoreBillingHelper() {
                            @Override
        ...
        authStateManager = AuthStateManager.getInstance(InstrumentationRegistry.getTargetContext());
    ...
    public void testAuthResponse() {
        ...
        AuthorizationRequest.Builder authRequestBuilder = new AuthorizationRequest.Builder(
                ...
                Uri.parse(APP_REDIRECT_SCHEME))
                .setScope(SCOPE)
                .setState(STATE);

        //test successful response
        AuthorizationResponse.Builder authResponseBuilder = new AuthorizationResponse.Builder(authRequestBuilder.build());
        authResponseBuilder.setAuthorizationCode(UUID.randomUUID().toString());
        authResponseBuilder.setScope(SCOPE);
        authResponseBuilder.setState(STATE);
        authStateManager.updateAfterAuthorization(authResponseBuilder.build(), null);

        assertNull(authStateManager.getCurrent().getAuthorizationException());

        //test oauth error
        authStateManager.clearState();
        authStateManager.updateAfterAuthorization(null, AuthorizationException.AuthorizationRequestErrors.CLIENT_ERROR);
        assertNotNull(authStateManager.getCurrent().getAuthorizationException());
    ...
    public void testTokenResponse() {
        ...
        //store successful authentication response
        AuthorizationResponse.Builder authResponseBuilder = new AuthorizationResponse.Builder(authRequestBuilder.build());
        authResponseBuilder.setAuthorizationCode(UUID.randomUUID().toString());
        ...
        //test successful token bearer response
        TokenRequest.Builder tokenRequestBuilder = new TokenRequest.Builder(config, CLIENT_ID);
        tokenRequestBuilder.setGrantType(TOKEN_TYPE_BEARER);
        TokenResponse.Builder tokenResponseBuilder = new TokenResponse.Builder(tokenRequestBuilder.build());
        tokenResponseBuilder.setAccessToken(UUID.randomUUID().toString());
        tokenResponseBuilder.setTokenType(TOKEN_TYPE_BEARER);
        tokenResponseBuilder.setRefreshToken(UUID.randomUUID().toString());
        tokenResponseBuilder.setIdToken(UUID.randomUUID().toString());
        tokenResponseBuilder.setAccessTokenExpiresIn(TimeUnit.DAYS.toMillis(730)); //2 year expiry
        authStateManager.updateAfterTokenResponse(tokenResponseBuilder.build(), null);

        assertNotNull(authStateManager.getCurrent().getAccessToken());
        assertNotNull(authStateManager.getCurrent().getRefreshToken());
        assertNull(authStateManager.getCurrent().getAuthorizationException());
        ...
        authStateManager.clearState();
        authStateManager.updateAfterAuthorization(authResponseBuilder.build(), null);
        tokenRequestBuilder.setGrantType(TOKEN_TYPE_REFRESH);
        tokenRequestBuilder.setRefreshToken(UUID.randomUUID().toString());
        TokenResponse.Builder tokenRefreshResponseBuilder = new TokenResponse.Builder(tokenRequestBuilder.build());
        tokenRefreshResponseBuilder.setAccessToken(UUID.randomUUID().toString());
        tokenRefreshResponseBuilder.setTokenType(TOKEN_TYPE_REFRESH);
        tokenRefreshResponseBuilder.setRefreshToken(UUID.randomUUID().toString());
        tokenRefreshResponseBuilder.setIdToken(UUID.randomUUID().toString());
        tokenRefreshResponseBuilder.setAccessTokenExpiresIn(TimeUnit.DAYS.toMillis(730)); //2 year expiry
        authStateManager.updateAfterTokenResponse(tokenRefreshResponseBuilder.build(), null);
        ...
        authStateManager.updateAfterTokenResponse(null, AuthorizationException.TokenRequestErrors.CLIENT_ERROR);

        //test oauth error token response
        authStateManager.updateAfterTokenResponse(null, AuthorizationException.TokenRequestErrors.OTHER);
        assertNull(authStateManager.getCurrent().getAccessToken());
        assertNotNull(authStateManager.getCurrent().getAuthorizationException());
        assertNotNull(authStateManager.getCurrent().getRefreshToken());
    }
